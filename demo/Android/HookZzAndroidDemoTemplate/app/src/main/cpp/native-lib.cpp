...
#define TEST_send 1
#define TEST_sendto 0
#define TEST_connect 0
#define TEST_freeaddrinfo 0
#define TEST_getaddrinfo 0
#define TEST_case_001 0
// check r7 save & restore
#define TEST_case_002 0

extern "C"
JNIEXPORT jstring

JNICALL
Java_com_example_z_hookzzandroiddemotemplate_MainActivity_stringFromJNI(
        JNIEnv *env,
        jobject /* this */) {
...
void common_pre_call(RegState *rs, ThreadStackPublic *ts, CallStackPublic *cs, const HookEntryInfo *info) {
    LOGI("<<< common_pre_call");
}

void common_post_call(RegState *rs, ThreadStackPublic *ts, CallStackPublic *cs, const HookEntryInfo *info) {
    LOGI("<<< common_post_call");
}

#if TEST_case_002
#if defined(__arm64__) || defined(__aarch64__)
void printf_pre_call(RegState *rs, ThreadStackPublic *ts, CallStackPublic *cs, const HookEntryInfo *info) {
    puts((char *)rs->general.regs.x0);
    STACK_SET(cs, (char *)"format", rs->general.regs.x0, char *);
    puts(">>> printf-pre-call");
}
#else

void printf_pre_call(RegState *rs, ThreadStackPublic *ts, CallStackPublic *cs, const HookEntryInfo *info) {
    char *origin_fmt = (char *) rs->general.regs.r0;
    if (!strncmp(origin_fmt, "HookZz", 6)) {
        rs->general.regs.r7 = 1;
        char *fake_fmt = (char *) "fake_fmt:HookZzzzzzz, %d, %p, %d, %c, %d, %d, %d, %d, %d\n";
        puts("origin printf fmt is: ");
        puts(origin_fmt);
        puts("change it with:");
        puts(fake_fmt);
        rs->general.regs.r0 = (unsigned long) fake_fmt;
...
void printf_post_call(RegState *rs, ThreadStackPublic *ts, CallStackPublic *cs, const HookEntryInfo *info) {
    if (STACK_CHECK_KEY(cs, (char *)"format")) {
        char *format = STACK_GET(cs, (char *)"format", char *);
        puts(format);
    }
...
__attribute__((constructor)) void test_hook_printf() {
    ...
    if (freopen("/data/data/com.example.z.hookzzandroiddemotemplate/test.output", "w", stdout) ==
        NULL)
        __android_log_print(ANDROID_LOG_DEBUG, "HOOK_TESTER", "%s", "open stdout file error!");

    ZzHook((void *) printf_ptr, NULL, NULL, printf_pre_call,
           printf_post_call, false);

    printf("HookZzzzzzz, %d, %p, %d, %d, %d, %d, %d, %d, %d\n", 1, (void *) 2, 3, (char) 0x41,
           (char) 5,
           (char) 6, 7, 8, 9);
    fclose(stdout);
}
...
void getaddrinfo_pre_call(RegState *rs, ThreadStackPublic *ts, CallStackPublic *cs, const HookEntryInfo *info) {
    LOGI(">>> getaddrinfo_pre_call");
}

void send_pre_call(RegState *rs, ThreadStackPublic *ts, CallStackPublic *cs, const HookEntryInfo *info) {
    LOGI(">>> send_pre_call!");
